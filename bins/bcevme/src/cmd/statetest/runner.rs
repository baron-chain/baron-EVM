...
fn check_evm_execution<EXT>(
    ...
    // If we expect exception bcevm should return error from execution.
...
pub fn execute_test_suite(
    ...
    for (name, unit) in suite.0 {
        ...
        let mut cache_state = bcevm::CacheState::new(false);
        for (address, info) in unit.pre {
            let acc_info = bcevm::primitives::AccountInfo {
        ...
        for (spec_name, tests) in unit.post {
            ...
            for (index, test) in tests.into_iter().enumerate() {
                ...
                cache.set_state_clear_flag(SpecId::enabled(
                    spec_id,
                    bcevm::primitives::SpecId::SPURIOUS_DRAGON,
                ));
                let mut state = bcevm::db::State::builder()
                ...
                let state = bcevm::db::State::builder()
                    .with_cached_prestate(cache)
                    .with_bundle_update()
                    .build();
